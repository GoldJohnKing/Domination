	...
	d_airai_attack_plane = call {
		...
		if (d_enemy_side_short == "W") exitWith {
			call {
				...
				if (d_rhs) exitWith {
					["RHS_A10","rhsusf_f22","JS_JC_FA18F"]
				};
				if (d_csla) exitWith {
					[]
				};
	...
	d_light_attack_chopper = call {
		...
		if (d_enemy_side_short == "W") exitWith {
			call {
				...
				if (d_rhs) exitWith {
					["RHS_MELB_AH6M","RHS_UH1Y_d","RHSGREF_A29B_HIDF","I_Plane_Fighter_03_dynamicLoadout_F","RHS_AH1Z","I_Plane_Fighter_04_F"]
				};
...
if (hasInterface) then {
	...
	call {
		...
		if (d_rhs) exitWith {
			["rhs_tigr_sts_msv", "AGE_LSV", "O_LSV_02_AT_F", "rhsgref_nat_uaz_spg9", "RHS_Ural_Ammo_MSV_01", "RHS_Ural_Repair_MSV_01", "RHS_Ural_Zu23_MSV_01", "O_Truck_02_medical_F", "rhsgref_BRDM2_ATGM_msv", "rhs_btr80a_vdv", "rhs_bmd4_vdv", "rhs_bmp2k_vdv", "rhs_bmp3m_msv", "rhs_sprut_vdv", "O_ZTL11", "rhs_2s3_at_tv", "rhs_t90a_tv", "rhs_t80u","rhs_t72be_tv"]
		};
		if (d_ifa3lite) exitWith {
